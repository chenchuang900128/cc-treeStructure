<template>
	...
	<view style="width: 1px; padding-bottom: 0px; margin-left: 12px; background-color: #C7C7C7;position: absolute;">
		...
		<view v-for="(item, index) in treeData" :key="bindKey(index)">

			<CCBCashSubTree @openClick="openChange" :tag="index" :subTreeData="item">
			</CCBCashSubTree>
			...
			<view v-if="item.isOpen" style="margin-top: 10px; margin-left: 10px;">
				...
				<view style="margin-left: 0px;" v-for="(subitem, otherIndex) in item.invests"
					:key="bindKey(200 + otherIndex)">

					<CCBCashBasTree @openClick="openChangeTwo" :tag="otherIndex" :subIndex="index"
						:subTreeData="subitem"></CCBCashBasTree>

					<!-- 父视图竖线 -->
					<view v-if="(subitem.invests!== undefined && subitem.invests.length > 0 && subitem.isOpen)"
						:style="{'height': bindSubLineHeight(subitem.invests) + 'px'}" class="subLineV">
					</view>

					<view v-if="subitem.isOpen" style="margin-top: 10px; margin-left: 10px;">

						<!--  sonIndex 修复小程序bug -->
						<view style="margin-left: 0px;" v-for="(sonitem, sonIndex) in subitem.invests"
							:key="bindKey(400 + sonIndex)">

							<CCBCashBasTree style="width: calc(100vw - 52px);" :subTreeData="sonitem"
								:tag="200 + sonIndex">
							</CCBCashBasTree>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openChangeTwo(item, subIndex, fatherIndex) {

				console.log("点击成功 = " + JSON.stringify(item));
				console.log("点击成功 fatherIndex = " + JSON.stringify(fatherIndex));
				console.log("点击成功 subIndex = " + JSON.stringify(subIndex));
				...
				// 父组件数据更新
				this.treeData[fatherIndex][subIndex] = item;

				let curItem = this.treeData[fatherIndex];

				this.treeData[fatherIndex] = curItem;
				// // 重新刷新组件 修复微信小程序
				this.$forceUpdate();
