<template>
	...
	<view class="sonV" :style="{'borderColor': (subTreeData.investType ==  0) ? '#4473FF': '#41D380'}">
		...
		<view style="display: flex;">
			...
			<image class="imgIcon" @click="openClick(subTreeData)" style="margin-left: 8px;" mode="aspectFit"
				:src="bindImgSrc()"> </image>
	...
	export default {
		...
		methods: {
			openClick(subItem) {
				...
				this.$forceUpdate();
				this.$emit("openClick", subItem, this.tag, this.subIndex);
			...
			bindImgSrc() {
				...
				if ((typeof(this.subTreeData.invests) == 'object') && (this.subTreeData.invests.length > 0)) {
